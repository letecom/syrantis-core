...
export function createDraftRoutes(dependencies: DraftRoutesDependencies = {}) {
  ...
  routes.post("/", async (c) => {
    ...
    const result = await draftService.createDraft(getWorkspaceId(c), c.get("userId"), parsedBody.data);
  ...
  routes.patch("/:id", async (c) => {
    ...
    const result = await draftService.updateDraft(getWorkspaceId(c), c.get("userId"), draftId, parsedBody.data);
